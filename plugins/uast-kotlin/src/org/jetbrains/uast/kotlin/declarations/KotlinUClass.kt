...
open class KotlinUClass private constructor(
    ...
    class KotlinPrimaryConstructorUMethod(
        ...
        override val uastBody: UExpression? by lz {
            ktClass.getAnonymousInitializers().takeIf { it.isNotEmpty() }?.let {
                KotlinLazyUBlockExpression(this@KotlinPrimaryConstructorUMethod) { getConstructorExpressions() }
            }
        }

        private fun getConstructorExpressions(): List<UExpression> = ktClass.getAnonymousInitializers().map {
...
class KotlinScriptUClass(
    ...
    class KotlinScriptConstructorUMethod(
        ...
        override val uastBody: UExpression? = KotlinLazyUBlockExpression(this) {
            script.declarations.filterIsInstance<KtScriptInitializer>().mapNotNull {
                it.body?.let { getLanguagePlugin().convert<UExpression>(it, this) }
            }
        }
    }
}